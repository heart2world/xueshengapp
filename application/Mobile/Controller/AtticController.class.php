...
class AtticController extends BaseController
{
    ...
    public function note_collect(){
        if(IS_POST){
            ...
            if(!$note){
                $this->ajaxReturn(['status'=>0,'info'=>'当前讨论不存在或已被禁用']);
            }
            //判断是否已经收藏
            $collect = M('UsersScripture')->where(array('user_id'=>$user_id,'scripture_id'=>$note_id,'type'=>1))->find();
            if($collect){//取消收藏
                $result = M('UsersScripture')->where(array('id'=>$collect['id']))->delete();
                if($result !== false){
                ...
                $dataInfo = [
                    'user_id' => $user_id,
                    'scripture_id' => $note_id,
                    'type' => 1,
                    'create_time' => time()
                ];
                $result = M('UsersScripture')->add($dataInfo);
                if($result !== false){
                    M('Scripture')->save(array('id'=>$note_id,'collect'=>$note['collect']+1));
    ...
    public function exchange(){
        ...
        $goods = M('Gift')->where(array('id'=>$goods_id,'status'=>0))->field('id,gift_name,price,surplus,cover_img')->find();
        if(!$goods){
