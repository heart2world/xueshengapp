...
class SchoolController extends AdminbaseController{
    ...
    public function profession(){
        ...
        $count = M('SchoolProfessional')->where($where)->count();
        $page = $this->page($count, 20);
        $profession = M('SchoolProfessional')->where($where)->limit($page->firstRow, $page->listRows)->select();
        foreach ($profession as $k=>$v){
            $user_count = M('Users')->where(array('specialty_id'=>$v['id']))->count();
            $profession[$k]['number'] = $user_count;
        }
