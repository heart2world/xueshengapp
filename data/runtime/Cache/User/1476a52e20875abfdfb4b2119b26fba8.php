		...
		<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
				...
				<td>
					<?php if($vo["user_type"] == 3): ?>——
						<?php else: ?>
						<?php echo ($verify_type[$vo['verify']]); endif; ?>
				</td>
				...
				<td class="action">
					<?php if($vo['user_status'] == 1): ?><a href="<?php echo U('indexadmin/ban',array('id'=>$vo['id']));?>" class="js-ajax-dialog-btn" data-msg="确定要停用该用户？">正常</a>
						<?php else: ?>
					<a href="<?php echo U('indexadmin/cancelban',array('id'=>$vo['id']));?>" class="js-ajax-dialog-btn" style="color: red;" data-msg="确定要将该用户恢复正常？">停用</a><?php endif; ?>
				</td>
