...
<div class="wrap js-check-wrap">
	...
	<form class="well form-search" method="post" autocomplete="off" action="<?php echo U('User/index');?>">
		...
		新增时间：
		<input type="text"  style="width: 150px" value="<?php echo I('request.time');?>" name="time" id="time"  placeholder="开始时间"  />—
		<input type="text"  style="width: 150px" value="<?php echo I('request.end_time');?>" name="end_time" id="end_time"  placeholder="结束时间"  />
			...
			<?php if(is_array($users)): foreach($users as $key=>$vo): ?><tr>
				...
				<td width="180">
					...
					<?php else: ?>
						...
						<a href="<?php echo U('user/reset_password',array('id'=>$vo['id']));?>" class="js-ajax-dialog-btn" data-msg="确定重置该用户密码？">重置密码</a><?php endif; ?>
				</td>
